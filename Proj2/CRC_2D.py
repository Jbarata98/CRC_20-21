...
def populate_2D():
    matrix = [[[resident_p, resident_q,0,[],(column,row)] for row in range(GRID_SIZE)] for column in range(GRID_SIZE)]

    for i in range(-1,2):
        matrix[int(GRID_SIZE/2) - 1][int(GRID_SIZE/2) + i - 1] = [mutant_p, mutant_q,0,[],(int(GRID_SIZE/2) - 1,int(GRID_SIZE/2) + i - 1)]
        matrix[int(GRID_SIZE/2) - 2][int(GRID_SIZE/2) + i - 1] = [mutant_p, mutant_q,0,[],(int(GRID_SIZE/2) - 2,int(GRID_SIZE/2) + i - 1)]
        matrix[int(GRID_SIZE/2) + 0][int(GRID_SIZE/2) + i - 1] = [mutant_p, mutant_q,0,[],(int(GRID_SIZE/2) + 0,int(GRID_SIZE/2) + i - 1)]
    return matrix
            ...
            while i < len(popl[row_nr][column_nr][3]):
                ...
                if r < comparing:
                    if i == 0:
                        ...
                        popl_new[row_nr][column_nr] = [rand.uniform(popl[row_nr][column_nr][0] - p_mut, popl[row_nr][column_nr][0] + p_mut),
                        ...
                        popl_new[row_nr][column_nr] = [rand.uniform(popl[n[1][0]][n[1][1]][0] - p_mut, popl[n[1][0]][n[1][1]][0] + p_mut),
...
def paint(lista):
    ...
    # print(len(lista[0))
    for r in range(GRID_SIZE * GRID_SIZE): # adicionar um parametro a nossa lista que era o index dessa posicao para esta parte
        x = lista[-1][r][4][0]

        y = lista[-1][r][4][1]
        patch[r] = ax.add_patch(patches.Rectangle([x,y] , 1, 1, color=(1-(lista[0][r][0]+lista[0][r][0]),0,0)))  # a cor é calculada com 1 - (p+q)
...
def main():
    ...
    def animate(i):
        ...
        for j in range(GRID_SIZE * GRID_SIZE):
            ...
            patch[j].set_color((1 - (tipo1+tipo2), 0, 0)) #(1-p+q)
        return patch
    ...
    anim.save('animacao_simulação.gif',writer=LoopingPillowWriter(fps = 30)) #you can change the fps for easier understanding
