...
def plot_degree_maker(n_nodes_degs,n_nodes_degs_prefattach,p_cum,p_cum_rand,POWER_LAW = False):
...
def prefattach_simulator(N, m, m0, a):
    if m < 1 or m >= N or a < 0:
        raise nx.NetworkXError(" network must have m >= 1"
                               ", alpha > 0 "
                               " and m < n, m = %d, n = %d, alpha=%d" % (m, N, a))
    ...
    for i in range(m0, N):
        ...
        if len(G.edges) == 0:
            ...
            total = sum([pow(G.degree(k), a) for k in G.nodes])
            problist = [pow(G.degree(k), a ) / total for k in G.nodes]
